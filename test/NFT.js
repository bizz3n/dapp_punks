...
describe('NFT', () => {
  ...
  describe('Minting', () => {
  	 ...
  	 beforeEach(async () => {
    	const NFT = await ethers.getContractFactory('NFT')
  		...
  		result = await transaction.wait()
  	 	...
  	 	it('returns the total number of the tokens the minter owns', async () => {
  	 	...
  	 	// EG: 'ipfs://QmQPEMsfd1tJnqYPbnTQCjoa8vczfsV1FmqZWgRdNQ7z3g/1.json'
  	 	 ...
  	 	 //console.log(await nft.tokenURI(1))
  	 	  expect(await nft.tokenURI(1)).to.be.equal(`${BASE_URI}1.json`)
  	 	})
     ...
     describe ('Failure', async () => {
    	...
    	it('it does not allow more NFTs to be minted than max amount', async () => {
     	const ALLOW_MINTING_ON = Date.now().toString().slice(0, 10) // Now
     	const NFT = await ethers.getContractFactory('NFT')
   	 	nft = await NFT.deploy(NAME, SYMBOL, COST, MAX_SUPPLY, ALLOW_MINTING_ON, BASE_URI)
  	 ...
  	 beforeEach(async () => {
    	const NFT = await ethers.getContractFactory('NFT')
  		...
  		result = await transaction.wait()
  	 	...
  	 	it('returns all of the NFTs for a given owner', async () => {
